...
def process_audio(audio_file, open_api_key, user_id, list_id):
    """
    Process the audio file and return the chatbot's response.
    """
    try:
        print("running process_audio", audio_file, open_api_key, user_id, list_id)
        ...
        save_task_to_database(bot_message, user_id, list_id)
...
def save_task_to_database(bot_message, user_id, list_id):
    """
    ...
    try:
        ...
        for task in tasks:
            if task:
                ...
                if len(task_parts) == 2:
                    ...
                    task_collection_lef = get_task_collection_lef(user_id, list_id)

                    # Create a new document reference in the "tasks" collection
                    task_ref = task_collection_lef.document()
        ...
        print("An error occurred while storing tasks in Firestore.")
        print(e)
